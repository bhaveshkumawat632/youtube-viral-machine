...
def test_combination_fallback_cascade_to_local_loop():
    # Cross-feature: Tier-1 failure -> stock failure -> local or original
    # FFmpeg motion succeeds. Patch the actual seam used by the pipeline so the
    # zero-cost Pollinations Tier-1 fallback is also considered unavailable.
    with patch(
        "modules.cloud_video_generator.generate_video_from_prompt_hf",
        return_value=None,
    ):
        with patch.dict(os.environ, {"PEXELS_API_KEY": ""}):
            with patch("urllib.request.urlopen", side_effect=Exception("Coverr offline")):
                ...
                os.makedirs(os.path.dirname(vidrush_pipeline.MANIFEST_FILE), exist_ok=True)
                if os.path.exists(vidrush_pipeline.MANIFEST_FILE):
                    os.remove(vidrush_pipeline.MANIFEST_FILE)
                ...
                # Check manifest.json: the result must be an approved local or
                # deterministic motion source, never an unrelated stock clip.
                assert os.path.exists(vidrush_pipeline.MANIFEST_FILE)
                with open(vidrush_pipeline.MANIFEST_FILE, "r") as f:
                    manifest = json.load(f)
                    assert len(manifest) >= 1
                    assert manifest[-1]["source"] in [
                        "local", "ffmpeg_motion", "ffmpeg_synthetic"
                    ]
