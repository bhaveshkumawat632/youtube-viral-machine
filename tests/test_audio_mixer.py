...
def mock_ffmpeg_success():
    ...
    with patch("subprocess.run") as mock_run, patch(
        "modules.audio_mixer._probe_levels",
        return_value={"max_volume": -1.0, "mean_volume": -14.0},
    ):
...
def test_mix_audio_sfx_missing_optional_keys(mock_ffmpeg_success):
    ...
    # The SFX chain applies a high-pass filter before the default gain.
    filter_complex = cmd[cmd.index("-filter_complex") + 1]
    assert "adelay=0|0,highpass=f=100,volume=0.5" in filter_complex
...
def test_audio_mixer_main_block():
    ...
    with patch("builtins.print") as mock_print:
        ...
        mock_print.assert_any_call("Cinematic Audio Mixer module — import to use.")
...
def test_mix_audio_ffmpeg_command_structure(mock_ffmpeg_success):
    ...
    with patch("os.makedirs") as mock_makedirs:
        ...
        # The broadcast voice chain must start at input 0 and include compression.
        assert "[0:a]highpass=f=80" in filter_complex
        assert "acompressor=threshold=-18dB" in filter_complex
        # BGM volume must be mapped to input 1
        assert "[1:a]volume=0.35" in filter_complex
        # Sidechain compress ducks the BGM using the processed voice.
        assert "[bgm_init][voice]sidechaincompress" in filter_complex
        # SFX tracks use a high-pass filter after their delays.
        assert "[2:a]adelay=1500|1500,highpass=f=100,volume=0.3" in filter_complex
        assert "[3:a]adelay=500|500,highpass=f=100,volume=0.8" in filter_complex
