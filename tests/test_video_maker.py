...
def test_create_video_format_video(tmp_path):
    ...
    with patch("modules.video_maker.check_copyright_killswitch", return_value=True), \
        ...
        # The prepared background already owns the 1920x1080 geometry. Verify
        # the horizontal progress-bar geometry and that the invalid full-video
        # zoompan filter is not reintroduced during final assembly.
        final_cmd = " ".join(mock_run.called_commands[-1])
        assert "drawbox=x=0:y=1074" in final_cmd
        assert "setsar=1" in final_cmd
        assert "zoompan" not in final_cmd
