...
def test_log_asset_happy():
    ...
    # Manifest lives under the isolated per-test assets dir (see conftest
    # isolate_vidrush_mutable_paths); never the checkout.
    manifest_path = vidrush_pipeline.MANIFEST_FILE
...
def test_check_ffmpeg_runtime():
    from vidrush_pipeline import check_ffmpeg_runtime
    # Should succeed on system with ffmpeg
    assert check_ffmpeg_runtime() is True

def test_check_ffmpeg_runtime_missing(monkeypatch):
    from vidrush_pipeline import check_ffmpeg_runtime, ALERT_FILE
    monkeypatch.setattr("shutil.which", lambda name: None)
    with pytest.raises(RuntimeError) as exc:
        check_ffmpeg_runtime()
    assert "FFmpeg binary missing" in str(exc.value)
    assert os.path.exists(ALERT_FILE)
...
def test_run_qa_gate_happy(tmp_path):
    ...
    manifest_data = [
        ...
        {"asset_id": "2", "source": "pexels.com", "license_type": "pexels_license", "url": "mock"},
