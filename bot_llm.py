...
def ensure_single_instance(lock_path="/tmp/telegram_bot.lock"):
    """
    Ensures only one instance of bot_llm runs at a time using POSIX fcntl advisory locking.
    Keeps lock file descriptor open for process lifetime. Exits cleanly (code 0) if locked.
    """
    global _LOCK_FILE_OBJ
    try:
        _LOCK_FILE_OBJ = open(lock_path, "w")
        fcntl.flock(_LOCK_FILE_OBJ.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
        _LOCK_FILE_OBJ.seek(0)
        _LOCK_FILE_OBJ.truncate()
        _LOCK_FILE_OBJ.write(f"{os.getpid()}\n")
        _LOCK_FILE_OBJ.flush()
    except (IOError, OSError):
        print("Another instance of bot_llm.py is already running. Exiting.")
        sys.exit(0)
...
if not BOT_TOKEN:
    botenv_path = os.path.join(BASE, ".botenv")
    if not os.path.exists(botenv_path):
        botenv_path = os.path.join(BASE, ".env")
    if os.path.exists(botenv_path):
        for line in open(botenv_path):
            line = line.strip()
...
def main():
    ensure_single_instance("/tmp/telegram_bot.lock")
    if not BOT_TOKEN:
