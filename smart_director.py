    ...
    while True:
        ...
        if frame_count % 3 == 0:
            ...
            if len(person_boxes) > 0:
                ...
                # If we have at least 2 people, assign left and right
                if len(sorted_boxes) >= 2:
                    left_person_box = sorted_boxes[0]
                    right_person_box = sorted_boxes[-1]
                elif len(sorted_boxes) == 1:
                    # Only one person detected, update the one it overlaps with most
        ...
        # Calculate motion for left and right
        if prev_gray is not None and left_person_box is not None and right_person_box is not None:
            def get_motion(box):
                ...
                face_y2 = y1 + int((y2 - y1) * 0.35) # Top 35% is face/mouth
            ...
            if cooldown > 0:
                ...
                # Switch logic: one must be significantly moving more than the other
        ...
        # Update target_x based on active speaker
        if active_target == "left" and left_person_box is not None:
            target_x = (left_person_box[0] + left_person_box[2]) / 2.0
        elif active_target == "right" and right_person_box is not None:
            target_x = (right_person_box[0] + right_person_box[2]) / 2.0
            
        # Super smooth camera pan
        current_x = current_x * 0.96 + target_x * 0.04
