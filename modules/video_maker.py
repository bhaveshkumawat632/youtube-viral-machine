...
def create_video_from_audio_and_subtitles(
    ...
    # Remove invalid zoompan on full video, just scale/crop if necessary, but bg_input is already sized
    # We just apply subtitles, eq, vignette, and progress bar
    filter_parts.append(f"[0:v]setsar=1")
...
def _prepare_background_video(video_path, width, height, duration):
    ...
    if vid_duration < duration:
        # Loop video with subtle Ken Burns zoom effect
        loop_count = math.ceil(duration / vid_duration)
        zoom_expr = "zoom=1.001:1.005"
        pan_expr = "x='iw/2-(iw/zoom/2)+sin(in*0.02)*40':y='ih/2-(ih/zoom/2)+cos(in*0.025)*30'"
        vf = f"scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height},{zoom_expr},{pan_expr},setsar=1,eq=contrast=1.05:saturation=1.15,fade=t=in:st=0:d=0.3,fade=t=out:st={duration-0.3}:d=0.3"
        cmd = [
            "ffmpeg", "-y",
...
def _prepare_multi_background_videos(video_paths, width, height, duration):
    ...
    # Concatenate the clips using the concat filter
    inputs = []
    for tc in temp_clips:
        inputs.extend(["-i", tc])
    
    parts = []
    for i in range(len(temp_clips)):
        parts.append(f"[{i}:v]")
    concat_filter = "".join(parts) + f"concat=n={len(temp_clips)}:v=1:a=0[v]"
