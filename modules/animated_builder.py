...
# animated background: dynamic moving gradient
BG = "gradients=s=1080x1920:c0=0x0f0c29:c1=0x302b63:c2=0x24243e:x0=0:y0=0:x1=1080:y1=1920:d=1"
...
def build_animated_scene(scene_text, audio_path, out_path, duration, pose="host_neutral"):
    ...
    _run(["ffmpeg", "-y", "-f", "lavfi", "-i", BG,
          ...
          "-c:v", "libx264", "-preset", "medium", "-b:v", "8M", "-maxrate", "10M", "-bufsize", "16M",
    ...
    # Pass through video directly without burning redundant captions
    cmd = ["ffmpeg", "-y", "-i", tmp_base]
    if audio_path:
        cmd += ["-i", audio_path, "-map", "0:v", "-map", "1:a", "-c:a", "aac", "-ar", "48000", "-b:a", "192k", "-shortest"]
    else:
        cmd += ["-map", "0:v", "-an"]
    cmd += ["-c:v", "libx264", "-preset", "medium", "-b:v", "8M", "-maxrate", "10M", "-bufsize", "16M",
