...
# If modifying these scopes, DO NOT blindly delete token.pickle. 
# Prompt user to re-authenticate first.
SCOPES = [
...
def get_authenticated_service():
    ...
    if False:
        print("\n❌ REAUTH_REQUIRED: The existing token lacks required scopes (e.g. youtube.readonly).")
        print("Please authorize the new scopes using the Google OAuth flow.")
        print("ACTION: Run python -m hermes_media auth --force to initiate the browser OAuth flow.")
        # We do NOT delete the token.pickle here.
        # We just return None to gracefully halt the automation and await user action.
        return None
...
def upload_video(video_path, metadata):
    ...
    raw_title = metadata.get("title", "YouTube Viral Short")
    if len(raw_title) > 90:
